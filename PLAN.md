...
- **Editable player country** (user request 2026-09-09, a replacement player from Poland): the country code on the Teams page is clickable → 3-letter input with suggestions from `PadelCountries.ISO3_TO_ISO2`; command `setPlayerCountry {teamId, player, country}` (upper-cased, must match /^[A-Z]{3}$/, '' clears) stores `teams_registry[id].countries`; the server attaches `resolvedCountry`, `rosterPlayers()` exposes `originalCountry`, the selected pair's flag updates live, pickers follow; ↺ now restores both name and country. Unknown codes are accepted but show no flag (add the SVG + map entry in `countries.js`).
- **Match start time on the players intro** (user request 2026-09-09): new `display.startTime` ('' by default, set through `setDisplay`, kept by resetMatch) edited with a time picker "Match start time" in the Overlay display card of the Score settings page; `/intro` shows it as a gold "START 14:30" pill at the right of the header (hidden while empty). The scorebug itself does not show it.
- **Scorebug hugs the names** (user request 2026-09-05): `.scoreboard` lost its `min-width: 400px`, so with short names and no completed sets the gold/white columns sit right after the longest name (only the serve-dot slot remains). `max-width: 660px` + ellipsis still cap very long names.
- **Scorebug 10% smaller** (user request 2026-09-05, from the first live picture): `overlay.js` applies a fixed `BASE_SCALE = 0.9` transform to the board (origin follows the `?pos=` corner); `?scale=` multiplies it, so the Admin/embed builders' "1" is the new default look. Court TV is unaffected (it skips the positioning code).
- **Editable player names on the Teams page** (user request 2026-09-04, night): click a name or ✎ → inline input (Enter saves, Esc cancels, blur saves), ↺ restores the entry-list name; corrected names get a dotted underline. Command `setPlayerName {teamId, player, name}` (`''` clears) stores overrides in `state.teams_registry[id].names` (persisted with the state, survives restarts and rebuilds; `setTeamActive` now preserves the entry). The server resolves the team (unknown ids ignored) and attaches `resolvedName`, so a pair already selected for the current match is renamed live (scorebug/intro/referee follow); `/api/teams` and `selectTeam` apply the overrides (`rosterPlayers()`, players carry `originalName`); the Score settings pickers read the override from the registry and rebuild on change. Not part of undo. Unit + e2e tests added.
- **Roster refreshed from the final entry lists** (user request 2026-09-04, evening; men V5 + women v2, both "Last Update: 4/9/2026 h 06:00 pm"): `data/teams.json` regenerated (87 pairs / 174 players / 29 countries, was 89/178/30). Men main draw: Simone Vaccari / Matteo Rosingana (ITA, was MD-23) withdrew; Filippo Nicocia / Matteo Platania moved up from qualifying #1 to main-draw position 26; Fernandez Sanchez/Bogarin, Muesser/Hugounenq and Rodriguez Vidal/Sebber Gormsen shifted to 23–25; wild cards 27–28 unchanged. Men qualifying: Mark Konjar / Jure Tozon (SLO, was Q-25) withdrew; every remaining pair moved up (now Q-01..Q-28; the operator's pair is M-Q-22). Women: 31 pairs, no changes. No ranking/points changes. The §11 listing below is the original 31 Aug snapshot — the JSON file is authoritative. Because ids are position-based, a match selected before the refresh keeps its names but its teamIds now point to other pairs → reset the match after restarting the server.
- **Roster cut to the semifinalists** (user request 2026-09-12, from `ORDER-OF-PLAY-SATURDAY-12-SEPTEMBER-2026-V2.pdf`, released 11 Sep 2026 7:27 PM): `data/teams.json` now holds only the 8 pairs on Center Court on Saturday — women SF Bellver Fructuoso/Dominguez Gracia [3] vs Martinez/Luján Rodríguez [2] (1:00 PM) and Martinez Gomez/Manquillo Alarza [1] vs Casali/Aima [4] (NB 2:30 PM); men SF Cepero/Aliaga [4] vs Aragon Herrera/Roglan Pons [5] (NB 4:00 PM) and Jensen Sirvent/Gala [1] vs Iacovino/Graziotti (NB 5:30 PM). Ids, positions, countries and rankings are kept from the entry list (M-MD-01/05/07/14, W-MD-01/02/05/08); a new `seed` field carries the bracket number from the PDF; player names follow the PDF spelling (Carlotta Casali, Clarissa Aima — the entry list had the longer Casali Vannicelli / Margherita Aima). All 8 are `active`. Empty sections/optgroups (qualifying) are already skipped by the Teams page and the admin pickers. The e2e test now uses M-MD-14 (Graziotti rename) vs M-MD-01. The PDF is Identity-H encoded, so the text was extracted with a small Node script (union of the embedded ToUnicode CMaps) rather than a PDF library.
- **Commercials folder + "Check a video"** (user request 2026-09-15, spots showed black in OBS on the new PC: `MP: Failed to open media: C:Padel...`): the spot list used absolute `C:PadelFipGoldBucharestCommercials…` paths written on the streaming PC. `server.js` now has a `commercialsDir` setting (`""` = auto: `COMMERCIALS_DIR` env, else the nearest `Commercials` folder above `Scoreboard` — the repo folder both from the source tree and from `binDebug
- **Instant replay on the Media tab** (user request 2026-09-15): new **Replay** card between Broadcast and Commercials. Built on OBS's own Replay Buffer via obs-websocket: `replayHousekeeping()` (3 s after start, then every 15 s) connects to OBS and runs `ensureReplayBuffer()` — pushes `replaySeconds` into the profile (`SetProfileParameter` RecRBTime for SimpleOutput + AdvOut, harmless for the unused mode), restarts the buffer when the length changed, starts it when it is off, and reports a readable error when it will not start (Replay Buffer not enabled in OBS's output settings — enabling needs OBS's settings dialog, the API cannot create the output). Command `saveReplay`: `SaveReplayBuffer`, poll `GetLastReplayBufferReplay` until a new path appears (15 s cap), wait for the file size to settle, move the clip from OBS's recording folder into the replay folder (rename, copy+unlink across drives; `(n)` suffix on a name clash) — OBS's own recording path is left untouched — then `runReplayClip()` plays it: fit-to-canvas, `SetInputSettings` on the replay media source, switch to the replay scene, wait for the media to end (cap = length + 15 s), back to the live scene; the scorebug is not touched (the replay scene simply does not contain it). `playReplay {file}` plays any clip from the folder by file name (no traversal). `GET /api/replays` = folder, length, newest 10 clips; `obs.replay` in the status payload (`enabled, seconds, bufferActive, saving, playing, last, count, error`) — the Media tab reloads the list when `count` changes. `breakState.kind` marks a running replay vs break (they share the running lock; Cancel = `cancelCommercials`). Settings: `replayEnabled` / `replaySeconds` (5–600) / `replayDir` ('' = nearest Replay folder above Scoreboard\, like the commercials) / `replayScene` (REPLAY) / `replaySource` (Replay) in obs-settings.json, editable in the Media settings card. `fitMediaSourceToCanvas` and `waitForCommercialsEnd` took scene/source/cap parameters. e2e: the mock OBS gained the replay-buffer requests (save = write a file into a fake recording folder); tests cover buffer start with the configured length, save → move → REPLAY/LIVE switch → listing, newest-first order, and `playReplay` by name with traversal rejected. README: Media tab bullet + OBS setup step 4.
- **`/replay` tag page removed** (user request 2026-09-17, after the cover was set up in OBS): `replay.html/.css/.js`, the friendly route, the Admin tab row and the e2e checks are gone; the replay cover `/overlay?cover=1` is the only replay graphic. The entry below is kept for history.
- **Replay tag page `/replay`** (user request 2026-09-17, "how can viewers tell a replay from live"): new `replay.html/.css/.js` — a transparent Browser Source for the OBS REPLAY scene (above the Replay media source, sized like the canvas). A small badge in the scorebug palette (blue gradient, gold left bar, CSS-drawn pulsing rewind chevrons, "REPLAY" in Bahnschrift, the tournament title `display.title` underneath) slides in from the nearest side edge while `obs.replay.playing` is set in the main-port state broadcast and slides out when the clip ends; all sizes are em-based from `font-size: min(1.35vw, 2.4vh)`. URL options: `?pos=top-left|top-right|bottom-left|bottom-right`, `?scale=`, `?title=0`, `?preview=1` (always visible, to position it in OBS). Friendly route `/replay` on the main port only — it is deliberately not in `PUBLIC_PAGES`/`PUBLIC_ASSET` (an OBS-only source, nothing for the website). Admin tab "OBS overlay URL" card gained the URL row (Copy / Preview opens `?preview=1`). e2e: main port serves the page + assets, public port answers 404. README: page bullet + OBS setup step 4 (also suggests a Stinger/Swipe Transition Override on the scene).
- **Scorebug hidden during a replay** (user request 2026-09-17): `runReplayClip()` remembers whether the score was showing, sends `setDisplay {scoreVisible:false}` + a 0.5 s fade before the scene switch, and restores `scoreVisible:true` after the clip (also on error/cancel) only if it was showing before — a score the operator had hidden stays hidden. This supersedes the earlier note that the replay does not touch the scorebug. e2e: hidden while the REPLAY scene is on, back afterwards, and stays hidden when it was hidden before. Also 2026-09-17: **Smart App Control** blocks the unsigned `FipGoldBucharest.exe` on the operator PC (CodeIntegrity 3077/3033) — the app is started through the signed host (`dotnet FipGoldBucharest.dll`, desktop shortcut to dotnet.exe, Run: Minimized); that launch only works for a DLL Smart App Control already accepts: a rebuild with a changed `MainWindow.xaml.cs` (a tray-icon tweak) was blocked (`FileLoadException 0x800711C7`, CodeIntegrity 3077 on the DLL), reverting the C# change and rebuilding reproduced the accepted file (deterministic build, same hash) and the app started again. **Rule on the operator PC: do not change the C# code** (`*.cs`, `*.xaml`, csproj) unless the binaries get signed or Smart App Control is turned off; everything under `Scoreboard` can change freely. README has the note.
- **Replay cover `/overlay?cover=1`** (user report 2026-09-17: "still shows the score on replay"): hiding the live scorebug cannot help — OBS's replay buffer records the **program picture**, so the score is baked into every clip (the operator's REPLAY scene holds only the mic, the `/replay` tag and the Replay media source; verified over obs-websocket). Fix without plugins: `overlay.js` cover mode — the same page, markup and `?pos=`/`?scale=` handling as the live scorebug, so the geometry is identical (measured 295.0 × 170.6 px at 40,40 for both), but `.board` gets an opaque background, its children `visibility:hidden` (layout kept) and an absolutely positioned `.cover-plate` (blue gradient, gold left bar, CSS rewind chevrons, "REPLAY"); the title bar and winner banner render as usual (identical to the recording underneath). Visibility follows `obs.replay.playing` instead of `display.scoreVisible` (`&preview=1` = always on). OBS setup: duplicate the LIVE scene's scorebug Browser Source into the REPLAY scene above the media source, set the cover URL, paste the original's transform. Admin tab: a Replay cover URL row that follows the scorebug's position/scale pickers (Copy / Preview). Limits: the cover is laid out from the current state, so if the score layout changed since the clip was recorded (a set column added) the widths can differ by a column; a true clean feed would need a per-source recording plugin (e.g. Source Record).
- **Match format card on the Score settings page** (user request 2026-09-15): two selects sent with `setConfig` — **Deuce (40-40)**: Star point (`deuceMode: star`, default; two advantage deuces then the decisive point, label `SP`), Golden point (`golden`; the point at 40-40 wins, label `GP`) or Killer point (`silver`; one advantage deuce `D1`, then the decisive point, label `KP`) — and **3rd set**: Normal set (`finalSetMode: normal`, default) or Maxi tiebreak (`superTiebreak` + `superTiebreakPoints: 10`, win by 2 — the engine already jumped straight into a super tiebreak at 1-1 in sets; `completeSet`/`resolveTiebreak` unchanged). Engine: `suddenDeathLabel(config)` picks SP/GP/KP in `pointLabel` (was always `SP`), exported `isSuddenDeathLabel(label)` drives the overlay's `.gp` styling for all three. The deuce rule applies immediately (labels recompute from the raw points); the 3rd-set choice must be made before the deciding set starts (a running deciding set is not converted). `resetMatch` keeps `config`, `resetAll` restores the defaults. Tests: killer point sequence, live `setConfig` switch + reset behaviour, maxi tiebreak 9-9 → 10-9 (not over) → 10-12 with the set recorded as `superTb`, and the normal 3rd set starting as games. README "Match format" section rewritten. Follow-up the same day (referee page showed the maxi set as 1-0): `resolveTiebreak` now records the super-tiebreak set as `gamesPerSet+1` / `gamesPerSet` (7-6 or 6-7) with `tb` = the tiebreak points and `superTb: true`, so every page (referee, overlay, TV, home) shows it like a set won on a tiebreak; the overlay also prints the tiebreak points under it (the `!set.superTb` exclusion was dropped). `removeLastSet` on a `superTb` set re-opens the maxi tiebreak at its final points instead of restoring 7-6 as games. `server.js` `normalizeSets()` rewrites a `superTb` set saved as 1-0 / 0-1 by an older build to 7-6 / 6-7 when the state file is loaded, so a finished match on screen updates after a restart. Note for operators: a `dotnet build` while the app is open fails at the exe copy **and skips copying `Scoreboard**` to `bin`**, so the restarted app keeps running the old server code — close the app before building (or copy `Scoreboard` into `binDebug
et10.0-windowsScoreboard` by hand and restart the score server from the Admin tab).
et10.0-windowsScoreboard`); `resolveSpotFile()` keeps an absolute path that exists, else uses the same file name inside that folder, so old settings files keep working without editing. Default spot files are plain names now. `GET /api/commercials` returns `dir`, `videos` (every `.mp4/.mov/…` in the folder, sorted) and each spot with its resolved `file` + `exists`. New command `playVideo {file}` (file name only, must be in the folder — no path traversal) runs the single-spot routine for any listed video (id `FILE:<name>`). Media tab: the six per-spot buttons were removed (follow-up request the same day) and replaced, in the same place under Play all / Cancel, by a dropdown of the folder's video files + a gold ▶ Play button (locked while a break runs, pulses while its file plays, "last played" shows the name); a **Commercials folder** field was added to the settings card (saving reloads the list). e2e: `/api/commercials` shape, `playVideo` swap/restore/scene switches, and rejection of `..server.js` / unknown names.
